...
public class Receiver {
	...
	@JmsListener(destination = "${destination.temp.sensor.queue}")
	public void receive(Map<String, String> message) throws ParseException {
		TemperatureSensor tempSensor = processIncomingMessage(message);
		LOGGER.info("---------'  received message='{}'", tempSensor.toString());
	}

	private TemperatureSensor processIncomingMessage(Map<String, String> message) {
		String sensorName = message.get("sensor");
		String value = message.get("value");
		if (value.length() > 15)
			value = value.substring(0, 15);
		String reportDate = message.get("time");
		if (Double.parseDouble(value) > 80.5)
			postToAnalomyQueue(sensorName, value, reportDate);
		TemperatureSensor tempSensor = tSensorManager.saveData(sensorName, null, reportDate, value);
	...
	@Value("${anomaly.temp.sensor.queue}")
